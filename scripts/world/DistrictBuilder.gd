...
func _build_ground() -> void:
	# Terreno base principal del valle
	_add_static_box("Valle Central", Vector3(0, -0.35, 0), Vector3(220, 0.7, 160), Color(0.24, 0.42, 0.22, 1.0))

	# Relieve topográfico 3D: Montañas, colinas, pendientes y depresiones naturales
	# Sierras de Managua (Montañas del Sur y Este)
	_add_terrain_hill("Montaña Sierras 1", Vector3(-70, 6.0, 65), Vector3(65, 12.0, 45), Color(0.18, 0.34, 0.16, 1.0))
	_add_terrain_hill("Montaña Sierras 2", Vector3(0, 8.5, 75), Vector3(90, 17.0, 50), Color(0.16, 0.32, 0.15, 1.0))
	_add_terrain_hill("Montaña Sierras 3", Vector3(75, 7.0, 60), Vector3(70, 14.0, 45), Color(0.20, 0.36, 0.18, 1.0))

	# Colinas y Pendientes Intermedias
	_add_terrain_hill("Colina Altagracia Oeste", Vector3(-80, 2.5, 10), Vector3(45, 5.0, 60), Color(0.22, 0.38, 0.20, 1.0))
	_add_terrain_hill("Pendiente Loma Tiscapa Base", Vector3(61, 2.8, 12), Vector3(42, 5.6, 38), Color(0.25, 0.40, 0.22, 1.0))

	# Costa y playa lacustre en depresión natural
	_add_static_box("Playa Xolotlán", Vector3(24, -0.015, -58), Vector3(180, 0.06, 20), SAND_COLOR, false)
	var water := _add_static_box("Lago Xolotlán", Vector3(24, -0.08, -80), Vector3(220, 0.08, 36), Color(0.06, 0.38, 0.58, 0.86), false)
	...
	if water_mesh and water_mesh.material_override:
		...
		water_mesh.material_override.metallic = 0.15

func _add_terrain_hill(label: String, center_pos: Vector3, size: Vector3, color: Color) -> void:
